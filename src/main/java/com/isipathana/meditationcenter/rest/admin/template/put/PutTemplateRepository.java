...
/**
 * Repository implementation for template update operations.
 *
...
public class PutTemplateRepository implements PutTemplateDataAccess {
    ...
    @Override
    public ScheduleTemplate updateTemplate(ScheduleTemplate template) {
        log.debug("Updating template: {}", template.templateId());
        ...
        var record = dslContext.update(SCHEDULE_TEMPLATE)
                ...
                .where(SCHEDULE_TEMPLATE.TEMPLATE_ID.eq(template.templateId()))
                .returning()
                .fetchOne();
